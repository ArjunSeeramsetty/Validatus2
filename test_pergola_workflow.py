#!/usr/bin/env python3
"""
Validatus2 Local Testing Plan for 5 Core Tasks - Pergola Case Study
====================================================================

This script tests the 5 core tasks of the Validatus2 platform using the Pergola market case study.
Based on the current application architecture, we'll test the existing endpoints and services.
"""
...
class PergolaWorkflowTester:
    ...
    def test_server_health(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.log("✅ Backend server is running and healthy")
                return True
            else:
                self.log(f"❌ Server health check failed: {response.status_code}", "ERROR")
                return False
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Cannot connect to backend server: {e}", "ERROR")
            return False
    
    def test_task1_topic_creation(self) -> bool:
        """Test Task 1: Topic Creation using sequential analysis endpoints"""
        self.log("🚀 Starting Task 1: Topic Creation")
        
        try:
            # Use the existing sequential analysis endpoint
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    self.log(f"✅ Topic created successfully: {topic_id}")
                    self.log(f"   Session ID: {self.session_id}")
                    self.test_results["task1"] = {"status": "success", "session_id": self.session_id, "topic_id": self.topic_id}
                    return True
                else:
                    self.log(f"❌ Topic creation failed: {result.get('message', 'Unknown error')}", "ERROR")
            else:
                self.log(f"❌ Topic creation request failed: {response.status_code}", "ERROR")
                self.log(f"   Response: {response.text}", "ERROR")
                
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Topic creation request failed: {e}", "ERROR")
    ...
    def test_task2_web_search(self) -> bool:
        ...
        if not self.session_id:
            self.log("❌ No session ID available for web search", "ERROR")
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/v3/sequential-analysis/analysis/{self.session_id}/stage1/start",
                timeout=30
            )
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get("success"):
                    self.log("✅ Stage 1 analysis started (includes web search)")
                    ...
                    self.log(f"❌ Stage 1 start failed: {result.get('message', 'Unknown error')}", "ERROR")
            else:
                self.log(f"❌ Stage 1 start request failed: {response.status_code}", "ERROR")
                self.log(f"   Response: {response.text}", "ERROR")
                
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Stage 1 start request failed: {e}", "ERROR")
    ...
    def _wait_for_stage1_completion(self, max_wait: int = 300) -> bool:
        ...
        while time.time() - start_time < max_wait:
            try:
                response = requests.get(
                    f"{self.base_url}/api/v3/sequential-analysis/analysis/{self.session_id}/stage1/status",
                    timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    if status == "completed":
                        self.log("✅ Stage 1 analysis completed successfully")
                        self.test_results["task2"] = {"status": "success", "stage1_completed": True}
                        return True
                    elif status == "failed":
                        self.log("❌ Stage 1 analysis failed", "ERROR")
        ...
        self.log("❌ Stage 1 analysis timed out", "ERROR")
    ...
    def test_task3_save_urls(self) -> bool:
        ...
        if self.test_results.get("task2", {}).get("status") == "success":
            self.log("✅ URLs automatically saved during Stage 1 analysis")
            self.test_results["task3"] = {"status": "success", "note": "Integrated with Stage 1"}
            return True
        else:
            self.log("❌ Task 3 failed because Stage 1 did not complete", "ERROR")
    ...
    def test_task4_content_scraping(self) -> bool:
        ...
        if self.test_results.get("task2", {}).get("status") == "success":
            self.log("✅ Content scraping automatically completed during Stage 1 analysis")
            self.test_results["task4"] = {"status": "success", "note": "Integrated with Stage 1"}
            return True
        else:
            self.log("❌ Task 4 failed because Stage 1 did not complete", "ERROR")
    ...
    def test_task5_vector_db(self) -> bool:
        ...
        if not self.session_id:
            self.log("❌ No session ID available for vector DB test", "ERROR")
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/v3/sequential-analysis/analysis/{self.session_id}/stage2/start",
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get("success"):
                    self.log("✅ Stage 2 started (includes vector DB creation and search)")
                    ...
                    self.log(f"❌ Stage 2 start failed: {result.get('message', 'Unknown error')}", "ERROR")
            else:
                self.log(f"❌ Stage 2 start request failed: {response.status_code}", "ERROR")
                self.log(f"   Response: {response.text}", "ERROR")
                
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Stage 2 start request failed: {e}", "ERROR")
    ...
    def _wait_for_stage2_completion(self, max_wait: int = 180) -> bool:
        ...
        while time.time() - start_time < max_wait:
            try:
                response = requests.get(
                    f"{self.base_url}/api/v3/sequential-analysis/analysis/{self.session_id}/stage2/status",
                    timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    if status == "completed":
                        self.log("✅ Stage 2 analysis completed successfully")
                        return True
                    elif status == "failed":
                        self.log("❌ Stage 2 analysis failed", "ERROR")
                        return False
                    else:
                        self.log(f"   Status: {status} - continuing to wait...")
                        time.sleep(5)
        ...
        self.log("❌ Stage 2 analysis timed out", "ERROR")
        return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "success":
                    ...
                    self.log("✅ Pergola market intelligence retrieved successfully")
                    ...
                    self.log("❌ Pergola intelligence request failed", "ERROR")
            else:
                self.log(f"❌ Pergola intelligence request failed: {response.status_code}", "ERROR")
                
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Pergola intelligence request failed: {e}", "ERROR")
    ...
    def test_semantic_search(self) -> bool:
        ...
        try:
            # Test pergola intelligence search
            search_queries = [
            ...
            for query in search_queries:
                ...
                if response.status_code == 200:
                    ...
                    self.log(f"✅ Search query: '{query}' - {len(results)} results found")
                else:
                    self.log(f"❌ Search failed for query: '{query}'", "ERROR")
            
            self.test_results["semantic_search"] = {"status": "success"}
            return True
            
        except requests.exceptions.RequestException as e:
            self.log(f"❌ Semantic search test failed: {e}", "ERROR")
            self.test_results["semantic_search"] = {"status": "failed"}
            return False
    ...
    def run_complete_workflow(self) -> Dict[str, Any]:
        ...
        if not self.test_server_health():
            self.log("❌ Cannot proceed - backend server is not running", "ERROR")
        ...
        for task_name, test_func in tasks:
            ...
            try:
                ...
                self.log(f"❌ {task_name} failed with exception: {e}", "ERROR")
    ...
    def generate_test_summary(self):
        ...
        self.log("\n🎯 Next Steps:")
        if success_rate >= 80:
            self.log("   ✅ Core workflow is functioning well")
            self.log("   🚀 Ready for production deployment")
        elif success_rate >= 60:
            self.log("   ⚠️  Some issues detected - review failed tests")
            self.log("   🔧 Consider debugging failed components")
        else:
            self.log("   ❌ Significant issues detected")
            self.log("   🛠️  Major debugging required before deployment")
        
        self.log("\n" + "=" * 60)

def main():
    """Main function to run the workflow test"""
    print("🧪 Validatus2 Pergola Workflow Test")
    print("=" * 50)
    ...
    results_file = f"pergola_workflow_test_results_{timestamp}.json"
