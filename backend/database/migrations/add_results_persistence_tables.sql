...
CREATE INDEX idx_factors_session ON computed_factors(session_id);
CREATE INDEX idx_factors_segment ON computed_factors(session_id, segment);
...
CREATE INDEX idx_patterns_session ON pattern_matches(session_id);
CREATE INDEX idx_patterns_segment ON pattern_matches(session_id, segment);

-- Monte Carlo Scenarios Storage
...
CREATE INDEX idx_scenarios_session ON monte_carlo_scenarios(session_id);
CREATE INDEX idx_scenarios_segment ON monte_carlo_scenarios(session_id, segment);

-- Consumer Personas Storage
...
CREATE INDEX idx_personas_session ON consumer_personas(session_id);

-- Segment Rich Content Storage
...
CREATE INDEX idx_rich_content_session ON segment_rich_content(session_id);
...
CREATE INDEX idx_results_status ON results_generation_status(session_id, status);
